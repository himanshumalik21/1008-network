...
export function FlowingMeshCanvas({ className }: FlowingMeshCanvasProps) {
  ...
  useEffect(() => {
    ...
    const waves = [
      {
        colorStops: [
          { pos: 0, color: "rgba(99, 91, 255, 0.20)" },
          { pos: 0.45, color: "rgba(0, 212, 178, 0.16)" },
          { pos: 1, color: "rgba(124, 58, 237, 0.10)" },
        ],
        edgeColor: "rgba(99, 91, 255, 0.50)",
        speed: 0.0035,
        offset: 0,
        startX: 0.72,
        endX: 0.90,
      ...
      {
        colorStops: [
          { pos: 0, color: "rgba(0, 212, 178, 0.16)" },
          { pos: 0.50, color: "rgba(16, 185, 129, 0.14)" },
          { pos: 1, color: "rgba(255, 112, 67, 0.12)" },
        ],
        edgeColor: "rgba(0, 212, 178, 0.45)",
        speed: 0.0045,
        offset: 2.5,
        startX: 0.58,
        endX: 0.75,
    ...
    const nodeColors = [
      "rgba(99, 91, 255, 0.65)",  // Indigo
      "rgba(0, 212, 178, 0.65)",  // Cyan
      "rgba(16, 185, 129, 0.60)", // Emerald
      "rgba(255, 112, 67, 0.60)", // Coral
    ];

    for (let i = 0; i < nodeCount; i++) {
      nodes.push({
        x: 0.45 + Math.random() * 0.52,
    ...
    function render() {
      ...
      try {
        ...
        // 1. Render Clean Silk Waves - Sweeping all the way down to bottom
        waves.forEach((w) => {
          ...
          const cp1y = height * (0.32 + Math.cos(t * 0.7) * 0.05 + mouseModY);

          const cp2x = width * (0.50 + Math.cos(t * 1.1) * w.amp - mouseModX * 0.3);
          const cp2y = height * (0.80 + Math.sin(t * 0.9) * 0.05);

          const endX = width * (w.endX + Math.sin(t * 0.75) * 0.06 + mouseModX * 0.2);
          const endY = height * 1.15; // Extends past bottom to touch footer
